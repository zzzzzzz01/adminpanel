<x-layout.app>
    <x-slot:title>
        {{ $group->group_name }} — {{ $groupSubject->subject->name_uz }} | Jurnal
    </x-slot:title>
                        ...
                        <table class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    ...
                                    @php
                                        $today = \Carbon\Carbon::today()->toDateString();       // faqat sana
                                        $scheduleDate = \Carbon\Carbon::parse($schedule->date)->toDateString(); // faqat sana

                                        if ($scheduleDate > $today) {
                                            $badgeClass = 'bg-success'; // kelajak
                                        } elseif ($scheduleDate < $today) {
                                            $badgeClass = 'bg-danger'; // o‘tib ketgan
                                        } else {
                                            $badgeClass = 'bg-primary'; // bugun
                                        }
                                    @endphp
                                        <th class="text-center" style="font-size: 14px; width: 20px; min-width: 97px; max-width: 20px;">
                                            <a href="{{ route('journalGroup.grades', $schedule->id) }}" class="text-decoration-none">
