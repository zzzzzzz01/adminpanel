    ...
    <div class="container mt-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                @if($submission->assignment->midtermInterval->type === 'manual')
                <li class="breadcrumb-item">
                    <a href="{{ route('midterms.manual', $submission->assignment->midtermInterval->id) }}"  class="text-decoration-none">
                        {{ $submission->assignment->midtermInterval->groupSubject->subject->{'name_' . app()->getLocale()} }} - {{ $assignment->midtermInterval->groupSubject->group->group_name }}
                    </a>
                </li>
                @elseif($submission->assignment->midtermInterval->type === 'assignment')
                <li class="breadcrumb-item">
                    <a href="{{ route('midterms.assignment', $submission->assignment->midtermInterval->id) }}"  class="text-decoration-none">
                        {{ $submission->assignment->midtermInterval->groupSubject->subject->{'name_' . app()->getLocale()} }} - {{ $submission->assignment->midtermInterval->groupSubject->group->group_name }}
                    </a>
                </li>
                @endif
                <li class="breadcrumb-item">
                    <a href="{{ route('assignments.submissions', $submission->assignment->id) }}"  class="text-decoration-none">
                    {{ $submission->assignment->name }}
                    </a>
                </li>
                <li class="breadcrumb-item">
                    <a href="" style="color: #808080;" class="text-decoration-none">
                    {{ $submission->student->last_name }}. {{ $submission->student->name }} 
                    </a>
                </li>
                            ...
                            @endphp
                                ...
                                @foreach($allSubmissions as $try)
                                    <tr>
                                        ...
                                        <td>
                                            @if($try->midtermGrade)
                                                {{ $try->midtermGrade->grade }} | {{ $try->assignment->max_score }}
                                            @else
                                                - | {{ $try->assignment->max_score }}
                                            @endif
