    ...
    <div class="container mt-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <a href="{{ route('groupSubject.grades', ['group' => $schedule->groupSubject->group->id, 'subject' => $schedule->groupSubject->subject->id]) }}" class="text-decoration-none">
                    {{ $schedule->groupSubject->group->group_name }} — {{ $schedule->groupSubject->subject->{'name_' . app()->getLocale()} }}
                    </a>
                </li>
                <li class="breadcrumb-item">
            ...
            <div class="col-md-12">
                <div class="card shadow-sm">
                    ...
                    <div class="card-body p-3">
                        <form action="{{ route('grades.store', $schedule->id) }}" method="POST">
                            @csrf
                                ...
                                <tbody>
                                    @foreach($students as $student)
                                        @php
                                            ...
                                            // Ball chegarasidan oshib ketganmi
                                            $disableInput = $student->total_current_score >= $maxCurrent;

                                            // Sana o'tib ketganmi
                                            $scheduleExpired = \Carbon\Carbon::parse($schedule->date)->isPast();

                                            // Umumiy disable shart
                                            $disableInput = $disableInput || $scheduleExpired;
                                        @endphp
                                        <tr>
